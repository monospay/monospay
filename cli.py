...
def main() -> None:
    ...
    try:
        match args.command:
            ...
            case "health":  cmd_health(args)
            case "status":  cmd_status(args)
            case "settle":  cmd_settle(args)
            case "nodes":
                match getattr(args, "nodes_cmd", None):
                    case "list":   cmd_status(args)
                    case "create": cmd_nodes_create(args)
                    case "kill":   cmd_nodes_kill(args)
                    case _:        p_nodes.print_help()
            case "config":
                match getattr(args, "cfg_cmd", None):
                    case "show":  cmd_config_show(args)
                    case "set":   cmd_config_set(args)
                    case "clear": cmd_config_clear(args)
                    case _:       p_cfg.print_help()
            case _:
                parser.print_help()
